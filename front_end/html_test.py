...
# env variables
KAFKA_TOPIC = 'task_signup'#os.getenv('KAFKA_TOPIC')
KAFKA_CONSUMER_GROUP_PREFIX = 'group' #os.getenv('KAFKA_CONSUMER_GROUP_PREFIX', 'group')
KAFKA_BOOTSTRAP_SERVERS = 'localhost:9092' #os.getenv('KAFKA_BOOTSTRAP_SERVERS', 'localhost:9092')
...
@app.get("/")
async def root():
    return {"message": "Hello World", "read" : _msg_read}
...
def form_post(request: Request, num: int = Form(...)):
    result = "unchaged state" if _state == 0 else str(_state)
    return templates.TemplateResponse('form.html', context={'request': request, 'result': result})
...
def _update_state(message: Any) -> None:
    ...
    _msg_read.append(value)
    number = int(value['id'][-1])
    log.info(f'Number from Value from msg: {number}')
    global _state
    _state = number
